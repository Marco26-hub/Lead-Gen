...
const FONTS = "https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,500;9..144,600;9..144,700&family=DM+Sans:wght@400;500;600;700&display=swap";
...
/** Pesce-Mare — bright airy coastal seafood. Split hero + wave, Fraunces serif. */
export function PesceMare({ model, rating, reviewCount, photos, openingHours }: DemoProps) {
        ...
        <div className={styles.content}>
          ...
          <section className={styles.hero} id="top">
            <div className={`${styles.wrap} ${styles.reveal}`}>
              <div>
                ...
                <h1 style={{ marginTop: 16 }}>{model.hero.headline}</h1>
                <p className={styles.lead}>{model.hero.sub}</p>
                <div className={styles.heroCta}>
                  <a className={`${styles.btn} ${styles.btnPrimary}`} href="#contatti">{model.hero.ctaLabel}</a>
                  {phone && <a className={`${styles.btn} ${styles.btnGhost}`} href={tel}>☎ {phone}</a>}
                  {rating != null && <span className={styles.ratingPill}><Stars /> <b>{rating.toFixed(1)}</b></span>}
                </div>
              </div>
              <div className={`${styles.heroPhoto} ${heroPhoto ? "" : styles.empty}`}>
                {heroPhoto && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={heroPhoto} alt={`${name} — il locale`} />
                )}
                {rating != null && (
                  <div className={styles.floatChip}><b>{rating.toFixed(1)} ★</b><small>{reviewCount ? `${reviewCount} recensioni` : "Recensioni Google"}</small></div>
                )}
              </div>
            </div>
            <svg className={styles.wave} viewBox="0 0 1440 60" preserveAspectRatio="none" aria-hidden="true"><path fill="currentColor" d="M0 40c180-30 360-30 540 0s360 30 540 0 240-30 360-20v60H0z" /></svg>
          </section>

          {/* pescato / servizi */}
          <section className={`${styles.block} ${styles.band}`} id="pescato" style={{ paddingTop: 24 }}>
            <div className={styles.wrap}>
              <div className={styles.head}>
                <span className={styles.eyebrow}>Il pescato</span>
                <h2>Dal mare alla tavola, ogni giorno</h2>
              </div>
          ...
          {menu && (
            <section className={`${styles.block} ${styles.menu}`} id="menu">
              <div className={styles.wrap}>
                <div className={styles.head}>
                  <span className={styles.eyebrow}>La carta del mare</span>
                  <h2>Il nostro menù</h2>
                  {menu.note && <p>{menu.note}</p>}
                </div>
          ...
          {/* gallery */}
          {gallery.length > 0 && (
            <section className={`${styles.block} ${styles.band}`} id="galleria">
              <div className={styles.wrap}>
                <div className={styles.head}><span className={styles.eyebrow}>Galleria</span><h2>Dai nostri tavoli</h2></div>
          ...
          {/* about */}
          <section className={`${styles.block} ${styles.band}`}>
                ...
                {hours.length > 0 && (
                  <div style={{ marginTop: 22, maxWidth: 380 }}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      {hours.map((h, i) => (<div className={styles.hoursRow} key={i}><span style={{ fontWeight: 600 }}>{h.day}</span><span style={{ opacity: .8 }}>{h.hours}</span></div>))}
                    </div>
                  </div>
                )}
              </div>
